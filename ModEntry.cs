        ...
        private void OnUpdateTicked(object sender, UpdateTickedEventArgs args)
        {
            ...
            if (args.IsMultipleOf(1))
            {
                ...
                if (Game1.activeClickableMenu is BobberBar bar)
                {
                    ...
                    double diffBobberFish = Math.Abs(bobberPosition - bobberBarPos);

                    double[] NewState = new double[] { (double)bobberBarPos, (double)bobberPosition, (double)bobberBarSpeed};


                   double reward = distanceFromCatching*0.8 + diffBobberFish< 100 ? 1 - (diffBobberFish / 448):-2; 
                    ...
                    // action_buffer: pressed -> TRUE, unpressed -> FALSE
                    // action: pressed -> 1, unpressed -> 0
                   // bool action = this.Helper.Input.IsDown(SButton.MouseLeft);
                    ...
                    if (updateCounter == bufferSize - 1 && shouldStoreDataset)
                    {
                        ...
                        File.WriteAllLines(datasetFile, iterRows);

                        this.Monitor.Log("Dumped 1000 entries in dataset" + this.Helper.ModRegistry.ModID,LogLevel.Info);
                        this.Monitor.Log("This is the " + times + " time",LogLevel.Info);
                        times++;   
                        Agent.reloadModel();                   
                        updateCounter=0;
                    }
